...
class _NetworkingTests(TestCase):
    ...
    def _verify_emptying(self, server, client):
        client.send('to connect'.encode())
        server.receive()
        client.send('before emptying'.encode())
        server.send('before emptying'.encode())
        time.sleep(0.01)
        client.empty()
        server.empty()
        client.send('after'.encode())
        server.send('after'.encode())
    ...
    def _assert_receive(self, receiver, msg):
        self.assertEqual(receiver.receive().decode(), msg)
...
class TestNetworking(_NetworkingTests):
    ...
    def test_server_send_udp(self):
        server, client = self._udp_server_and_client(ports['SERVER_PORT'], ports['CLIENT_PORT'])
        server.send_to('foofaa'.encode(), LOCAL_IP, ports['CLIENT_PORT'])
    ...
    def test_setting_port_no_ip(self):
        ...
        server.send_to('foofaa'.encode(), LOCAL_IP, client.get_own_address()[1])
        self._assert_receive(client, 'foofaa')

    def test_setting_ip_no_port(self):
        server, client = self._udp_server_and_client(ports['SERVER_PORT'], '')
        server.send_to('foofaa'.encode(), *client.get_own_address())
        self._assert_receive(client, 'foofaa')
