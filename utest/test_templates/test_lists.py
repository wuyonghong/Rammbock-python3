...
class TestListTemplate(TestCase):
    ...
    def test_create_list(self):
        ...
        self.assertEqual(list.name, 'topthree')
        self.assertEqual(list.encode({}, None)[0].int, 1)
        self.assertEqual(list.encode({}, None)[2].int, 1)

    def test_create_list_with_setting_value(self):
        list = get_list_of_three()
        encoded = list.encode({'topthree[0]': 42}, {}, None)
        self.assertEqual(encoded[0].int, 42)
        self.assertEqual(encoded[1].int, 1)

    def test_list_with_struct(self):
        list = get_struct_list()
        encoded = list.encode({'liststruct[1].first': 24}, {}, None)
        self.assertEqual(encoded[0].first.int, 1)
        self.assertEqual(encoded[1].first.int, 24)
        self.assertEqual(encoded[1].second.int, 2)

    def test_list_list(self):
        outerList = get_list_list()
        encoded = outerList.encode({'listlist[0][1]': 10, 'listlist[1][0]': 55}, {}, None)
        self.assertEqual(encoded[0][1].int, 10)
        self.assertEqual(encoded[1][0].int, 55)
        self.assertEqual(encoded[1][1].int, 7)
    ...
    def test_set_list_values_with_defaults(self):
        ...
        self.assertEqual(encoded.first_list[1].int, 2)
        self.assertEqual(encoded.second_list[1].int, 2)
        self.assertEqual(encoded.first_list[0].int, 42)
        self.assertEqual(encoded.second_list[0].int, 42)
